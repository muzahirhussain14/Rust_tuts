...
fn spawn_light_thread(receiver: Receiver<LightMsg>) -> JoinHandle<LightStatus> {
    ...
    thread::spawn(move || {
        ...
        loop {
            match receiver.recv() {
                Ok(msg) => match msg {
                    LightMsg::ChangeColor(r, g, b) => println!("Color changed to: {}", "       ".on_truecolor(r, g, b)),
                    LightMsg::On => {println!("Light Switched On");  light_status = LightStatus::Off;}
                    LightMsg::Off => {println!("Light Switched Off"); light_status = LightStatus::Off;}
                    LightMsg::Disconnect => {println!("Disconnected from the light buld"); break;}
                },
                _ => {
...
fn main() {
    ...
    let join_handler = spawn_light_thread(receiver);

    sender.send(LightMsg::On);
    sender.send(LightMsg::ChangeColor(123,255,200));
    sender.send(LightMsg::ChangeColor(125, 125, 125));
    sender.send(LightMsg::ChangeColor(200, 200, 200));
